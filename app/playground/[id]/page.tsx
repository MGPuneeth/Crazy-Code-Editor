...
import { useFileExplorer } from "@/modules/playground/hooks/use-fileExplorer";
...
import { useAiSuggestion } from "@/modules/playground/hooks/use-ai-suggestion";
  ...
  const handleSave = useCallback(
    async (fileId?: string) => {
      ...
      try {
        ...
        if (!filePath) {
          toast.error(
            `Could not find path for files: ${fileToSave.filename}. ${fileToSave.fileExtension}`
          );
          return;
        }
        ...
        const updatedFileContent = (items: any[]) =>
          ...
          items.map((item) => {
            if ("folderName" in item) {
              return { ...item, items: updatedFileContent(item.items) };
            } else if (
        ...
        updatedTemplateData.items = updatedFileContent(
                        ...
                        {openFiles.map((file) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className="ml-2 h-4 hover:bg-destructive hover:text-destructive-foreground rounded-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  closeFile(file.id);
                                }}
