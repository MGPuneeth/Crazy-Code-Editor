...
export async function POST(request: NextRequest) {
  try {
    const body: CodeSuggestionRequest = await request.json(); //getting the body of the code from the frontend
    ...
    if (!fileContent || cursorLine < 0 || cursorColumn < 0 || !suggestionType) {
      return NextResponse.json(
        { error: "Invealid input parameters" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        generratedAt: new Date().toISOString(),
      },
    });
  } catch (err: any) {
