...
const PlaygroundEditor = ({
  ...
  const generateSuggestionId = () =>
    `suggesiton-${Date.now()}-${Math.random()}`;
  ...
  const acceptCurrentSuggestion = useCallback(() => {
    ...
    if (
      ...
      console.log("❌ Cannot accept - missing refs");
      return false;
    }
    ...
    try {
      ...
      console.log("📍 Position check:", {
        currentLine: currentPosition.lineNumber,
        currentColumn: currentPosition.column,
        expectedLine: suggestionPos.line,
        expectedColumn: suggestionPos.column,
      });
      ...
      if (
        ...
        console.log("⚠️ Position changed, cannot accept suggestion");
        return false;
      }
      ...
      // Call the parent's accept handler (which clears the state in the hook)
  ...
  useEffect(() => {
    ...
    if (suggestion && suggestionPosition) {
      console.log(
        "✅ Registering new inline completion provider for suggestion"
      );
      ...
      // Show visual indicator with decorations
      if (editor && editor.deltaDecorations) {
          ...
          [
            {
              range: new monaco.Range(
                ...
                suggestionPosition.column,
                suggestionPosition.line,
                suggestionPosition.column + 1
              ),
              options: {
                glyphMarginClassName: "codicon codicon-lightbulb",
                glyphMarginHoverMessage: {
                  value: "💡 AI Suggestion Available (Press Ctrl+Space)",
                },
        ...
        console.log("Added visual indicator decoration");
      }

      // Trigger inline suggestions to show
      setTimeout(() => {
        if (
          editorRef.current &&
          !isAcceptingSuggestionRef.current &&
          ...
          console.log("⚡ Triggering inline suggestions display");
  ...
  const handleEditorDidMount = (editor: any, monaco: Monaco) => {
    ...
    console.log("Editorinstance mounted: ", !!editorRef.current);
